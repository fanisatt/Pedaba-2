...
if (undo==2) {
    ...
    do {
        sprintf(file_1,"%s%s%d%s",tmp_path,"undo",undo_lasfile,".dat");
    ...
    if (inp_mem_ghost) free(inp_mem_ghost);
    ghost=inp_mem_ghost=0;
    strcpy(file_1,tmp_path);
    strcat(file_1,"snapshot.dat");
    fpundo=fopen(file_1,"rb");
    if (fpundo!=NULL) {
        fread (&cur,2,1,fpundo);
        fread (&ar,2,1,fpundo);
...
if(!mem_tent) {printf ("Out of memory") ; free (mem1); free(mem_enter); free (mem_undscr) ; return 0;}
getimage(xep+182,yep+4,xep+223,yep+30,mem_tent);
size=imagesize (xr+3,yt-1,xr+13,yb);
mem_bar=malloc(size);
if (mem_bar==NULL) {printf ("Out of memory") ; free (mem1); free(mem_enter); free(mem_tent); free (mem_undscr) ; return 0; }
  ...
  if (vam[0]) {                              // βάφω 
    ...
    if(sdlev.button.button==SDL_BUTTON_LEFT) {   
        ...
        if (linker) {
            ...
            if ( (!clipbo) || (!clipbo1) ) {
                information(2-keyb_gr,NULL,NULL); 
                if (clipbo) free(clipbo);
                if (clipbo1) free(clipbo1);
                continue ; }
            i=0;
            ...
            uf=0;
            run_men[0] = 1 ;
            sprintf(mypath,"/home/%s/",user_name);
            th = Start_strstr(str1kb,mypath);
            if (th) { uf=1;}
            else { 
                th = Start_strstr(str1kb,"/mnt/");
                if (th) uf=1;
            }
            
            if (!uf) {
                th = Start_strstr(str1kb,"/media/");
                if (th) uf=1;
            }
            
            if (!uf) {
                th = Start_strstr(str1kb,"https://");
                if (th) uf=2;
            }
            
            switch (uf) {
                case 0 : // Δεν φαίνεται να είναι τοπικό αρχείο ή διαδικτυακός σύνδεσμος...
                    
                    run_men[1]=run_men[2]=run_men[3]=run_men[4]=run_men[5]=run_men[6]=run_men[7]=run_men[8]=run_men[9]=run_men[10]=0;
                    break ;
                    
                case 1 : // Δείχνει να είναι τοπικό αρχείο
                    
                    run_men[9] = run_men[10] = 0 ;
                    
                    if (access(str1kb, F_OK )) // DEN YPARXEI STO DISKO OMVS...
                    {
                        run_men[1]=run_men[2]=run_men[3]=run_men[4]=run_men[5]=run_men[6]=run_men[7]=run_men[8]=0;
                        break ;
                    }
                    
                    lp = check_imgtype(str1kb);
                    if (lp) {
                        
                        run_men[1] = (opt_men[1]) ? 1 : 0;
                        run_men[2] = (opt_men[2]) ? 1 : 0;
                        run_men[3] = (opt_men[3]) ? 1 : 0;
                        run_men[4] = (opt_men[4]) ? 1 : 0;
                        run_men[5]=run_men[6]=run_men[7]=run_men[8]=0;
                        break;
                    }
                    
                    lp = check_mmediatype(str1kb);
                    if (lp) {
                        
                        run_men[1]=run_men[2]=run_men[3]=run_men[4]=run_men[5]=run_men[6]=0;
                        run_men[7] = (opt_men[7]) ? 1 : 0;
                        run_men[8] = (opt_men[8]) ? 1 : 0;
                        break;
                    }
                    
                    // Αφήνω ανοιχτούς τους κειμενογράφους....αφού υπάρχει στο δισκο..
                    run_men[7]=run_men[8]=run_men[9]=run_men[10]=0;
                    run_men[1]=run_men[2]=run_men[3]=run_men[4]=0;
                    run_men[5] = (opt_men[5]) ? 1 : 0;
                    run_men[6] = (opt_men[6]) ? 1 : 0;
                    
                    break ;
                    
                case 2 : // Δείχνει να είναι διαδικτυακός σύνδεσμος
                    
                    run_men[1]=run_men[2]=run_men[3]=run_men[4]=run_men[5]=run_men[6]=run_men[7]=run_men[8]=0;
                    
                    run_men[9] = (opt_men[9]) ? 1 : 0;
                    run_men[10] = (opt_men[10]) ? 1 : 0;
                    break ;
            }
            
            if (keyb_gr) normtext_tomy ("Μενού επιλογών",str1024);
            else normtext_tomy ("Options Menu",str1024);
            user_ans=smmenu_show_opt(11, submenu, run_men, str1024, xl+37,yt-20);
            for (f=0 ; f<3 ; f++) { chxr[f] = proxchxr[f] ; bkxr[f] = proxbkxr[f] ; slxr[f] = proxslxr[f]; }
            if (user_ans<0) {free(clipbo) ; free(clipbo1) ; continue ;}
            ...
            switch (kyk) {
                ...
                case 1:
                    ...
                    fclose(fpundo);
                        ...
                        strcpy(file_2,tmp_path);
                        strcat(file_2,"snapshot.dat");
                        fpundo=fopen(file_2,"rb");
                        if (fpundo!=NULL) {fclose (fpundo); remove (file_2);}
                        
                        a=undo_lasfile-undo_max;
                        sprintf(file_1,"%s%s%d%s",tmp_path,"undo",a,".dat");
